...
presets.disconnectCliMakeSure = function () {
    GUI.timeout_add(
        "disconnect",
        function () {
            $("div.connection_button_wrapper a.connection_button__link").trigger("click");
        },
        500,
    );
};
...
presets.onSaveConfigClick = function () {
    ...
    this.activateCli()
        ...
        .then((cliStrings) => {
            ...
            return FileSystem.pickSaveFile(
                filename,
        ...
        .then(() => {
            ...
            this._domWarningFailedToLoadRepositories.html(
                i18n.getMessage("presetsFailedToLoadRepositories", {
                    repos: failedToLoad.map((repo) => repo.name).join("; "),
                }),
            );
...
presets.checkPresetSourceVersion = function () {
    ...
    return new Promise((resolve, reject) => {
        const differentMajorVersionsRepos = self.presetsRepo.filter(
            (pr) => self.majorVersion !== pr.index.majorVersion,
        );
...
presets.prepareFilterFields = function () {
    ...
    this.prepareFilterSelectField(
        ...
        getUniqueValues(this.presetsRepo, (x) => x.index.uniqueValues.category),
        3,
    );
    this.prepareFilterSelectField(
        this._selectKeyword,
        getUniqueValues(this.presetsRepo, (x) => x.index.uniqueValues.keywords),
        3,
    );
    this.prepareFilterSelectField(
        this._selectAuthor,
        getUniqueValues(this.presetsRepo, (x) => x.index.uniqueValues.author),
        1,
    );
    this.prepareFilterSelectField(
        this._selectFirmwareVersion,
        getUniqueValues(this.presetsRepo, (x) => x.index.uniqueValues.firmware_version),
        2,
    );
    this.prepareFilterSelectField(
        this._selectStatus,
        getUniqueValues(this.presetsRepo, (x) => x.index.settings.PresetStatusEnum),
        2,
    );
...
presets.displayPresets = function (fitPresets) {
    ...
    fitPresets.forEach((preset) => {
        const presetPanel = new PresetTitlePanel(
            this._divPresetList,
            preset[0],
            preset[1],
            true,
